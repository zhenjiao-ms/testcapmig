---
title: _abandoned - CAPS Features and Status
ms.custom: na
ms.reviewer: na
ms.suite: na
ms.tgt_pltfrm: na
ms.assetid: 8c906b05-915f-4882-9dbb-095d447c7473
---
# _abandoned - CAPS Features and Status
Phase 2 MRef Features
====
Phase 2 (through sprint 79) provides end-to-end mref support for standard (non-customized) projects, with some limitations (see below). As of sprint 78, most core features are implemented and in verification testing. 
...
---------|---------|---------|---------
[1419](https://capservice.visualstudio.com/DefaultCollection/CAPS/_workitems#_a=edit&id=1419)|Reflection can be configured via the CAPS client.|Implemented and verified.|
...
- **Headings**  
  You can either you header dropdown list provided in the authoring toolbar, or mannuly add '---' or '===' in the     next line under your section title. You can also create a heading by adding one or more `#` symbols before your     heading text. The number of `#` you use will determine the size of the heading.
...
- **Blockquotes**
  You can indicate block quates with a `>`.
  ```
...
---
- **Insert Links**
  You can create an inline link by wrapping link text in brackets `[Linke Label]`, and then wrapping the link in    parentheses `(http://LinkURL)`. 
  [Bing.com](http://www.bing.com)
  
  You can also create a link to an exsiting topic in CAPS with the authoring toolbar
  
- **Insert Image**
  You can reference a online image from external resouce by using `![Image Label]`, and then wrapping the image resource url in parentheses `(http://ImageURL)`
...
You can also include inline Markdown format syntax such as links, bold, italics, or strikethrough:
